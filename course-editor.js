...
<button class="editor-btn primary" id="exportJson">📤 Экспорт</button>
    ...
    bindFormEvents() {
        ...
        this.container.addEventListener('input', (e) => {
            const { target } = e;
            if (target.classList.contains('editor-textarea')) {
                // Сбросим высоту, чтобы избежать "залипания"
                target.style.height = 'auto';
                // Установим новую высоту, но не больше max-height
                const newHeight = Math.min(target.scrollHeight, 500);
...
`;
    ...
    getGuide() {
        return `
MindGym — это тренажёр для развития навыков через диалог, а не тест на знания.
Пользователь формулирует ответ сам, как в реальной жизни. ИИ даёт фидбек как наставник.

## ЦЕЛЬ КУРСА
Создать путь от "я не замечал" к "я вижу и умею", через:
- Понятные жизненные ситуации,
- Вопросы на применение, а не на воспроизведение,
- Короткие, уникальные эталонные ответы,
- Фидбек, который объясняет, а не хвалит.

## СТРУКТУРА КУРСА
Курс — JSON с полями: metadata, modules, exercises.

### metadata
- title: до 50 символов
- description: до 160 символов (SEO)
- version: "1.0"
...
- prompt: мини-объяснение (1–2 предложения) + вопрос на применение
- expected_answer: 3–8 слов, разговорный, уникальный для кейса
- feedback_on_error: **ТОЛЬКО объяснение сути**.  
  Это показывается, когда пользователь не смог ответить верно, а ИИ не смог его направить.  
  → НЕ начинай с "Верно", "Правильно", "Отлично", "Хорошая попытка".  
  → НЕ хвали, не оценивай эмоции.  
  → Просто объясни, почему правильный ответ именно такой.  
  Пример: "Код из СМС — это цифровой ключ от сейфа с вашими деньгами. Банк никогда не запрашивает его по телефону."
- difficulty: 1–4

## ПРАВИЛА СОЗДАНИЯ УПРАЖНЕНИЙ
1. В prompt — сначала контекст, потом вопрос на действие.  
   Пример: "Код из СМС — это как ключ от сейфа с деньгами. Почему нельзя назвать его по звонку?"
2. expected_answer должен быть коротким, простым и уникальным.  
   Пример: "Потому что это ключ от моих денег"
3. Избегайте:
   - Вопросов вида "Назовите 3 причины..." — пользователь не угадает нужный набор.
   - Проверки памяти ("Как зовут автора?").
   - Шаблонных ответов ("Да", "Нет", "Согласен").
4. Используйте метафоры в prompt, но не в expected_answer.
5. Пишите для человека без специальных знаний.

## ВАЖНО
- Все поля обязательны.
- expected_answer должен быть уникальным для кейса.
- feedback_on_error НЕ должен содержать похвалу или оценку усилий.
- Курс должен быть целостным: модули логически связаны, упражнения усложняются.

## ГИБКОСТЬ
Если пользователь в описании курса уточнил особые требования (например, использовать термины из узкой области), следуй этим указаниям — они важнее общих правил.
